...
void inicia_tabul(void) //inicia o tabuleiro com espaços vazios (tabul[i][j]=0) ou R (tabul[i][j]=1) ou G (tabul[i][j]=2)
{
  ...
  for(i=1; i<N+1; i++){
  	for(j=1; j<N+1; j++) {
		if(i==3&&j==4) tabul[i][j] = 2; //desenha os 13 gansos na posição inicial, que é a partir da linha 5 ate ao fim do tabuleiro 
		if(i>=5) tabul[i][j] = 1; //desenha a raposa na posição 3,4	
		}
	}      
}
